...
def stack_frame(frame: Dict[str, np.ndarray], features: Iterable[str]) -> np.ndarray:
    """Stacks the features of a frame supplied in dictionary format.

    Args:
        frame: A dictionary mapping feature names to the feature array.
        features: The features from the dictionary to be stacked.

    Returns:
        A numpy array with all the features stacked on top of each other.
    """
    return torch.stack([frame[feature] for feature in features], axis=0)
...
class LRUCache:
    ...
    def __init__(self, capacity: int, monitoring: bool = False) -> None:
        ...
        self.values: Ordereddict = OrderedDict()  # Dict[int, io.BufferedReader] = {}
