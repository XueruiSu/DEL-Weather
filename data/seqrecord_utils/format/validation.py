...
from climai_global.seqrecord.datapipes import build_wdatapipe
import torch
import numpy as np
...
@click.command()
@click.option("--container-dir", default="/datadrive/azure_storage/weathereastus/era5seqrecord")
@click.option("--reading-service", required=True, type=click.Choice(["mp", "dist", "mpdist"]))
...
def main(container_dir: str, reading_service: str, testing: bool = True):
    ...
    record: WSeqRecord = WSeqRecord.load_record(recorddir=recorddir)
    ...
    dp = build_wdatapipe([record], None, None, batch_size=1, mappings=[], collate_fn=identity)
