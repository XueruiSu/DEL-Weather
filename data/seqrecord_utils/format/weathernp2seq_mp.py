...
def weatherdata2seqrecord(
    ...
    sub_wsrecord = WSeqRecord(
        os.path.join(config["wseqrecord_dir"], WSeqRecord.subfolder_name(rank, world_size))
    )
...
if __name__ == "__main__":
    ...
    for i in range(config["num_processes"]):
        sub_weatherfile_generator = islice(weatherfile_generator, dividens[i][0], dividens[i][1])
        p = Process(
